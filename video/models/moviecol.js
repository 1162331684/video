...
MovieCol.getColMovieDetails = function(uid, callback){
    db.query('select movies.title as title, movies.id as id, movies.info as info, colmovie.addtime as addtime, colmovie.id as cid, movies.url as url from movies , colmovie WHERE movies.url = colmovie.movie_url and colmovie.user_id = ?', [
        uid
    ], function (err, result) {
        if (err) {
            return callback(err, null);
