...
exports.showIndex = function (req, res, next) {
    ...
    if (type === 'tv') {
        ...
        cartoon.getcartoonsByCurrentPage(params, function (err, result) {
            if (err) {
                return next(err);
            }
            result.forEach(function (element) {
            ...
            // 2. 获取电视列表的总数量信息
            cartoon.getcartoonsPageNums(function (err, pageInfo) {
                if (err) {
                    return next(err);
                }
                let pageNum = Math.ceil(pageInfo.pageNum / pageSize);
                return res.json({
                    code: 1,
                    cartoons: result,
                    cartoonPageNum: pageNum,
                });
            })
        })
    } else if(type === 'variety'){
        variety.getvarietysByCurrentPage(params, function (err, result) {
            if (err) {
                return next(err);
            }
            result.forEach(function (element) {
            ...
            // 2. 获取电视列表的总数量信息
            variety.getvarietysPageNums(function (err, pageInfo) {
                if (err) {
                    return next(err);
                }
                let pageNum = Math.ceil(pageInfo.pageNum / pageSize);
...
function getHotMovies(callback) {
    ...
    Preview.getPreview(function (err, previews) {
        ...
        // 开始随机获取8个电影
        Movie.getRandomMovies(8, function (err, result) {
            if (err) {
                callback(err, null);
            }
            // 修改url地址
