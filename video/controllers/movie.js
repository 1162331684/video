...
var id =req.body.id
    ...
    Movie.deletemovieById(id,function (err,result) {
        if (err){
            return next(err)
        }
        if (result){
            return  res.redirect('/admin/movielist/1');
        }
        res.json({
            code: 0,
            msg: 'failed'
        ...
        if (movies.length === 0) {
            Movie.getMovieByName(searchMoviename, function (err, result) {
                ...
                result.forEach(function (element) {
                    ...
                    if(element.info.toString().length > 70) {
                        element.info = element.info.toString().substring(0, 70);
                    }
                })
