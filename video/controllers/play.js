...
exports.showPlay = function (req, res, next) {
    ...
    Movie.getMovieByUrl(url, function (err, movie) {
        ...
        if (movie.length) {
            ...
            if(movie.score==0) {movie.score="无评分"}
        }
...
if(err){
    return next(err)
            ...
            Movie.updatecartoonPlayNumsByUrl(url,function (err,result) {
                if(err){
                    return next(err)
                }
            })
            Movie.updateTVPlayNumsByUrl(url,function (err,result) {
                if(err){
                    return next(err)
                }
            })
            Movie.updatevarietyPlayNumsByUrl(url,function (err,result) {
                if(err){
                    return next(err)
                }
            })
            // 获取评论信息数量
            Comments.getCommentNumsByurl(url ,function (err, pageInfo) {
                if (err) {
                    return next(err);
                }
                ...
                // 获取当前页面的所有评论信息
                Comments.getCommentByCurrentmovie(url,params, function (err, comments) {
                    if (err) {
                        return next(err);
                    }
